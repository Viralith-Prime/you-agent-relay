...
function handleMessage(message, senderPort, senderId) {
    ...
    switch (message.type) {
        case 'CONNECT':
            // Store platform/browser info
            if (message.platform) guardianState.platform = message.platform;
            if (message.browser) guardianState.browser = message.browser;
            break;
        ...
        case 'HEALTH_CHECK':
            ...
            senderPort.postMessage({
                ...
                activeChannels: guardianState.activeChannels.size,
                platform: guardianState.platform,
                browser: guardianState.browser,
                timestamp: Date.now()
            });
            break;
        ...
        case 'EXPORT_STATS':
            // Export injection statistics
            senderPort.postMessage({
                type: 'STATS_EXPORT',
                stats: getSerializableStats(),
                fullState: getSerializableState(),
                timestamp: Date.now()
            });
            break;
    }
